...
function sumOfCodes(str) {
  ...
  return str.split('').reduce((a, char) => a + char.charCodeAt(0), 0);
}
...
function countVowels(str) {
  if (str.length) {
    const result = str.match(/[aeiyou]/gi);
    if (result) {
...
function isPalindrome(str) {
  ...
  const checkStr = cleanStr.split('').reverse().join('');
...
function findLongestWord(sentence) {
  const longWord = sentence.split(' ').reduce(function word(acc, current) {
...
function invertCase(str) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    ...
    if (substr === substr.toUpperCase()) {
      array[i] = substr.toLowerCase();
    } else {
      array[i] = substr.toUpperCase();
    }
  }
  return array.join('');
}
